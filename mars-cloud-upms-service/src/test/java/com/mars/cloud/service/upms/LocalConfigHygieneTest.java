 ...
 * 否则那份文件会变成「我本机的配置」，别人克隆后拿到的是一份错的默认值。
 *
 * <p>断言按 **key** 做，不做字符串匹配：注释里出现 "password" 或示例写法都不算违规。
 */
@SpringBootTest
@ActiveProfiles({"local", "test"})
class LocalConfigHygieneTest extends com.mars.cloud.service.upms.security.SecurityTestSupport {
    ...
    @Test
    void localProfileStaysOfflineByDefault() {
        assertThat(environment.getActiveProfiles()).contains("local");
        assertThat(environment.getProperty("mars.upms.local-fixture.enabled")).isEqualTo("true");
