...
check "actuator/health 为 UP" "UP" \
  "$(curl -fsS "$BASE/actuator/health" | sed -n 's/.*"status":"\([A-Z]*\)".*/\1/p')"

echo
echo "② 成功路径：业务对象被包成信封"
body=$(curl -fsS "$BASE/v1/orders/1")
...
status=$(curl -s -o /tmp/sample-③.json -w '%{http_code}' -X POST "$BASE/v1/orders" \
  -H 'Content-Type: application/json' -d '{"sku":"out-of-stock","quantity":1}')
check "HTTP 状态码" "200" "$status"
contains "success 为 false" '"success":false' "$(cat /tmp/sample-③.json)"
contains "错误码为 66102" '"code":"66102"' "$(cat /tmp/sample-③.json)"

echo
echo "④ 资源不存在：HTTP 404"
status=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/v1/orders/missing")
check "HTTP 状态码" "404" "$status"

echo
echo "⑤ 参数校验失败：HTTP 400"
status=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/v1/orders" \
  -H 'Content-Type: application/json' -d '{"sku":"","quantity":0}')
check "HTTP 状态码" "400" "$status"

echo
echo "⑥ i18n：文案随 Accept-Language 变化"
# 该端点刻意返回 404，所以这里**不能**用 curl -f —— -f 会把响应体丢掉
zh=$(curl -sS -H 'Accept-Language: zh-CN' "$BASE/v1/orders/missing" | sed -n 's/.*"message":"\([^"]*\)".*/\1/p')
en=$(curl -sS -H 'Accept-Language: en-US' "$BASE/v1/orders/missing" | sed -n 's/.*"message":"\([^"]*\)".*/\1/p')
...
check "v3/api-docs 状态码" "200" \
  "$(curl -s -o /dev/null -w '%{http_code}' "$BASE/v3/api-docs")"
