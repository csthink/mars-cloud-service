...
| `SERVER_ADDRESS` | 业务端口、管理端口与注册到 Nacos 的地址，规则与其他部署物相同（[部署说明](../docs/deployment.md) 的「端口与 context path」）。部署时填实例的私网 IPv4 地址，不要绑公网，不填通配地址或主机名；在容器里运行时填容器在私网里的地址，默认的 `127.0.0.1` 在容器外不可达 |
