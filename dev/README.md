...
认证服务由自身 Flyway 维护应用表，需要空库或匹配的迁移历史。中间件创建的数据库若已有其他验证表，请保留原库并为认证验证另建专用库；不要开启自动 baseline 或删除未知数据。配置与 HTTPS 准备见 [认证服务 README](../mars-cloud-auth-service/README.md)。
