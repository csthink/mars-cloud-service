...
gateway 的路由表还包含认证、产品、订单与通知服务；`/upms/**` 与 `/sample/**` 在 local / test 可经回环 Host 访问，两个服务也都可以按各自端口直接访问。经 gateway 转发的检查放在下一小节，与带令牌的业务请求一起做。完整路由与跨域条件见 [网关模块说明](../mars-cloud-gateway/README.md)。
