...
#
# ⚠️ 还有一份**完整版**检查，它包含若干不适合公开的模式，因此只留在私有设计仓。
#    把那份脚本放进公开仓本身就会造成敏感内容外流——所以公开仓只有本脚本这一份。
#    本脚本是公开仓 hook 与 CI 的**唯一**实现。
...
#    「脚本坏了」——两种情况的输出都是「✅ 通过」。见私有仓 adr/0006-safety-scan-scope.md。
